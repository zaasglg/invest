...
class LocalChatService
{
    ...
    {
        return (bool) preg_match(
            '/(помощ|көмек|не білесің|не істей|что умееш|help|навигац|бөлім|раздел|меню|функци|мүмкіндік|возможност|сайт|жүйе|система)/ui',
            $query,
        );
    }
    ...
    public function analyzeQuery(string $query, ?User $user = null): array
    {
        ...
        if (preg_match('/(помощ|көмек|не білесің|что ум|help|навигац|бөлім|раздел|менюдегі|функци|сайт|жүйе|система)/ui', $query)) {
            $entities[] = 'help';
        }
