...
const REGION_METRICS: RegionMetric[] = [
    {
        ...
        target: 1.784,
        facts: [
            { value: '$2 млрд', label: 'ПИИ' },
            { value: '58', label: 'запущено проектов' },
            { value: '8 340', label: 'новых рабочих мест' },
        ],
    },
    {
        id: 'grp',
        ...
        test: true,
        facts: [
            { value: '+10,1%', label: 'реальный рост' },
            { value: '1,67 млн ₸', label: 'ВРП на человека' },
            { value: '334,9 млрд ₸', label: 'транспорт и склады' },
        ],
    },
    {
        id: 'market',
        ...
        test: true,
        facts: [
            { value: '2,147 млн', label: 'население региона' },
            { value: '74,6%', label: 'сельские жители' },
            { value: '4,3%', label: 'зарегистр. безработица' },
        ],
    },
    {
        id: 'industry',
        ...
        test: true,
        facts: [
            { value: '+50,8%', label: 'обработка · 6М 2026' },
            { value: '+32,4%', label: 'вся промышленность' },
            { value: '+19%', label: 'добывающий сектор' },
        ],
    },
    {
        id: 'agriculture',
        ...
        test: true,
        facts: [
            { value: '76%', label: 'теплиц Казахстана' },
            { value: '$355,5 млн', label: 'экспорт АПК' },
            { value: '214,4 млрд ₸', label: 'инвестиции в АПК' },
        ],
    },
    {
        id: 'fdi',
        ...
        test: true,
        facts: [
            { value: '$1,998 млрд', label: 'факт 2025' },
            { value: '121,6%', label: 'рост всех инвестиций' },
            { value: '111', label: 'проектов в портфеле' },
        ],
    },
    {
        id: 'trade',
        ...
        test: true,
        facts: [
            { value: '$189,3 млн', label: 'экспорт' },
            { value: '$130,1 млн', label: 'импорт' },
            { value: 'Узбекистан', label: 'прямой выход на рынок' },
        ],
    },
    {
        id: 'tourism',
        ...
        test: true,
        facts: [
            { value: '+15,9%', label: 'рост в 2025' },
            { value: '1,012 млн', label: 'исторические объекты' },
            { value: '36,3 млрд ₸', label: 'инвестиции · 5М 2026' },
        ],
    },
    {
        id: 'projects',
        ...
        test: true,
        facts: [
            { value: '2,8 трлн ₸', label: 'портфель 2026–2029' },
            { value: '66', label: 'запусков в 2026' },
            { value: '20 000', label: 'рабочих мест в плане' },
        ],
    },
    {
        id: 'jobs',
        ...
        test: true,
        facts: [
            { value: '8 340', label: 'создано в 2025' },
            { value: '20 000', label: 'план 2026–2029' },
            { value: '5 879', label: 'мест на промплощадках' },
        ],
    },
    {
        id: 'salary',
        ...
        test: true,
        facts: [
            { value: '+7,1%', label: 'рост за год' },
            { value: '187,7 тыс.', label: 'кадры в АПК' },
            { value: '98,4 тыс.', label: 'промышленность и стройка' },
        ],
    },
    {
        id: 'spaces',
        ...
        test: true,
        facts: [
            { value: '29', label: 'производственных площадок' },
            { value: '275', label: 'готовых зданий' },
            { value: '6%', label: 'льготная ставка' },
        ],
    },
];
...
const REGION_INFRASTRUCTURE: InfrastructureItem[] = [
    {
        ...
        decimals: 0,
        readiness: 95,
        detail: '1 181 км в работах · 283 объекта',
        scalable: true,
    },
    {
        id: 'rail',
        ...
        decimals: 1,
        readiness: 76,
        detail: 'выход на Узбекистан и магистрали РК',
        scalable: true,
        test: true,
    },
    {
        ...
        decimals: 0,
        readiness: 88,
        detail: '7 630 подстанций и трансформаторов',
        scalable: true,
    },
    {
        id: 'gas',
        ...
        decimals: 1,
        readiness: 85.5,
        detail: '87 проектов начато в 2025 году',
    },
    {
        id: 'sites',
        label: 'Промплощадки',
        ...
        decimals: 0,
        readiness: 82,
        detail: '275 готовых производственных зданий',
        scalable: true,
        test: true,
    },
    {
        id: 'utilities',
        label: 'Коммунальные проекты',
        value: 257,
        unit: 'объектов',
        decimals: 0,
        readiness: 49,
        detail: '126 объектов уже завершено',
        scalable: true,
    },
];
    ...
    return REGION_METRICS.map((metric, metricIndex) => {
        const series =
            metric.id === 'spaces'
                ...
                : metric.series.map((value) =>
                      Number(
                          (value * coefficient).toFixed(metric.decimals + 1),
                      ),
                  );
        return {
            ...metric,
            series,
            target: Number(
                (series.at(-1)! * (1.14 + metricIndex * 0.015)).toFixed(
                    metric.decimals + 1,
                ),
            ),
            test: true,
...
function sectorsFor(id: string): SectorReport[] {
    ...
    return REGION_SECTORS.map((sector) => ({
        ...sector,
        volume: Number(
            (sector.volume * coefficient).toFixed(sector.decimals + 1),
        ),
...
function infrastructureFor(id: string): InfrastructureItem[] {
    ...
    return REGION_INFRASTRUCTURE.map((item, index) => {
        const readiness = Math.min(
            ...
            Math.max(
                42,
                Math.round(item.readiness + ((seed + index * 7) % 17) - 8),
            ),
        );
        return {
            ...item,
...
export function RegionMap3D({ progress }: { progress: number }) {
    ...
    const metricGrowth = Math.round(
        (currentMetricValue / activeMetric.series[0] - 1) * 100,
    );
    const metricProgress = Math.min(
        100,
        Math.round((currentMetricValue / activeMetric.target) * 100),
    );
    const metricChartMax = Math.max(
        activeMetric.target,
        ...activeMetric.series,
    );
            ...
            >
                ...
                <div className="data-panel-heading">
                    ...
                    {!isWholeRegion && (
                        <button
                            type="button"
                            onClick={() => setSelectedId(REGION_ID)}
                        >
                            Вся область
                        </button>
                    )}
                    ...
                    <>
                            ...
                            {metrics.map((metric) => {
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() =>
                                            openSectorFromMetric(metric.id)
                                        }
                                    >
                        ...
                        <div className="region-chart-section">
                            <div className="chart-period">
                                ...
                                <strong>+{metricGrowth}%</strong>
                            </div>
                            <h3>{activeMetric.label}</h3>

                            <div className="metric-target">
                                <div className="metric-target-head">
                                    <span>Выполнение цели</span>
                                    <strong>{metricProgress}%</strong>
                                </div>
                                <div className="metric-target-track">
                                    <i
                            ...
                            <div
                                ...
                                aria-label={`Динамика показателя ${activeMetric.label}`}
                            >
                                {[
                                    ...activeMetric.series,
                                    activeMetric.target,
                                    ...
                                    <div
                                        key={index}
                                        className={
                                            index === activeMetric.series.length
                                                ? 'target'
                                                : ''
                                        }
                                    >
                                        ...
                                        <i
                                            style={{
                                                height: `${Math.max(8, (value / metricChartMax) * 82)}%`,
                                            }}
                                        />
                                        <span>
                                            {index ===
                    ...
                    <div className="sector-report-view">
                        ...
                        <div className="sector-detail">
                            ...
                            <div className="sector-kpis">
                                <div>
                                    <span>Объём отрасли</span>
                                    <strong>
                                        {formatMetric(
                                            activeSector.volume,
                                            activeSector.decimals,
                                        )}{' '}
                                ...
                                <div>
                                    <span>Сырьё / база</span>
                                    <strong>{activeSector.rawBase}</strong>
                                </div>
                            </div>
                            ...
                            <div className="sector-products-head">
                                ...
                                <b>
                                    {activeSector.test || !isWholeRegion
                                        ? 'Оценка'
                                        : 'Профиль'}
                                </b>
                            </div>
                            ...
                            <div className="sector-opportunities">
                                ...
                                {activeSector.opportunities.map((item) => (
                                    <b key={item}>{item}</b>
                                ))}
                            </div>
                        </div>
                ...
                {panelView === 'infrastructure' && (
                    <div className="infrastructure-view">
                        <div className="infrastructure-heading">
                            <div>
                                <span>Готовность площадки</span>
                                <h3>Инфраструктура для инвестора</h3>
                            </div>
                        ...
                        <div className="infrastructure-grid">
                            {infrastructure.map((item) => (
                                <article key={item.id}>
                                    ...
                                    <footer>
                                        <span>готовность / охват</span>
                                        <b>{Math.round(item.readiness)}%</b>
                                    </footer>
                                </article>
                            ))}
                        </div>
                        <div className="infrastructure-note">
                            Дороги · железная дорога · энергия · газ ·
                            промплощадки · коммунальные сети
                        </div>
                    </div>
                )}

                {!isWholeRegion && panelView === 'summary' && (
                    <button className="open-territory" type="button">
                        Открыть профиль <ArrowRight size={16} />
                    </button>
                )}
            </aside>

            <div className="explorer-source" style={{ opacity: panelReveal }}>
                Границы: геопортал области · Показатели: БНС и gov.kz ·
                Оценочные данные отмечены
            </div>
        </section>
    );
}
